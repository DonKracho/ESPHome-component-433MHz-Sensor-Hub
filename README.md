...
This project is for decoding 433MHz AM transmissions from GT-WT-02, AFT 77 A1 abd Digoo weather station transmitters.
There are a lot of identical transmitters sending a 37-bit protocol with checksum for temperature and humidity.

Later on I got a bunch of DIGOO DG-TH8898 weather stations including temperature/humidity and raingauge sensors.
Unfortunately their 36-bit protocol does not include a checksum, what makes it more complex to get reliable values.
This repository is configured to use these DIGOO sensors right now. But the sensor type can be set within the
...
The initial decoder source was designed for the Arduino IDE and an ESP8266 device and was migrated to a
ESPhome Sensor Hub component meanwhile. Before I implemented MQTT Sensors for HomeAssistant. But I like ESPHome
because it is easier to use and communicates with HomeAssistant faster.
...
To be honest, with the [RXB6](docs/rxb6_datasheet.pdf) I've got the most stable results and range of reception. I really tried a lot of these 433MHz AM receivers in the past, but **the RXB6 is the only one I recommend to use!**
...
The reception routine has been improved over the years and operates more reliably than the original weather stations
belonging to them. The received codes are stored in a ring buffer. At least two equal codes have to be recorded in a transmission block. It is possible to decode even interfered signals of different sensors.
...
and published to the corresponding sensors. The rain sensor has a history functionality to calculate the rate
...
low power rf sensors may be the smartest way to go. The sensors are cheap and accurate and designed for outdoor
use. The batteries last for at least 18 months. The resolution is much better than for Zigbee or BLE sensors and you'll get a continous update about every 70 to 90 seconds. Don't get me wrong, I'm a fan of using DIY sensors, but many existing products do the job better and at lower price.
